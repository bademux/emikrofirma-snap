    ...
    public boolean AIBY(String var1) {
        ...
        if (var1 != null) {
            if (!"brak".equalsIgnoreCase(var1) && !FCW.getInstance().getMessageForKey("micro.process.general.empty").equalsIgnoreCase(var1)) {
                ...
                boolean var4;
                if (!var3.matches()) {
                    org.slf4j.LoggerFactory.getLogger(getClass()).debug("assuming other NIP");
                    var4 = this.HOT(var1);
                    return var4;
                }

                if (var1.length() == 10) {
                    org.slf4j.LoggerFactory.getLogger(getClass()).debug("assuming standard PL NIP");
                    var4 = super.AIBY(var1);
                    return var4;
                }

                org.slf4j.LoggerFactory.getLogger(getClass()).debug("assuming prefixed PL NIP");
