...
public class QSD implements QRX<InvoiceElement> {
    ...
    public InvoiceElement RIB(InvoiceElement var1, InvoiceElement var2) {
        ...
        BigDecimal var6;
        if (var3) {
            var6 = EQY.HRH(var2.DBI().getValue(), var2.DBG().getValue());
            var2.DBK().setValue(var6);
        }

        if (var4) {
            var6 = BigDecimal.ZERO;
            if (!var2.DBN().getValue().DEQ() || !this.QWH) {
                var6 = var2.DBN().getValue().getRate().divide(new BigDecimal(100)).setScale(EQY.FYD);
            }

            var6 = EQY.HRH(var2.DBK().getValue(), var6);
            var2.DBL().setValue(var6);
        }

        if (var5) {
            var6 = BigDecimal.ZERO;
            BigDecimal var7 = var2.DBK().getValue();
            if (!var2.DBN().getValue().DEQ() || !this.QWH) {
                var6 = var2.DBL().getValue();
            }
        ...
        var2.RIG().setValue(CalculationMethodType.NET);
        var2.RIF().setValue(EQY.getBrutValue(var2.DBI().getValue(), var2.DBN().getValue()));
        return var2;
    }
}
