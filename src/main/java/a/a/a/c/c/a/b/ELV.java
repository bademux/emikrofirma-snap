   ...
   public File getOrSelectWorkingDir(final Stage var1) throws FFK {
      ...
      try {
         ...
         if (var3 != null && var3.DEY().getValue() != null) {
            try {
               var12 = new File((String)var3.DEY().getValue());
               if (this.RKD(var12)) {
                  var13 = false;
               }
            } catch (Exception var10) {
               EXF.getInstance().ICH("Profile working dir unavailable [" + (String)var3.DEY().getValue() + "]", var10);
            }
         }

         if (var13) {
            EXF.getInstance().ICE("Selecting new working directory");
            var12 = (File)FEL.IKS(new FEM<File>() {
               public File IKT() {
