...
public class FAC extends FAB<PasswordFieldValidated> {
   ...
   protected void IFV(String var1, String var2) {
      if (var2 != null && var2.length() > 0 && !var2.equals(var1)) {
         ...
         if (!var3) {
            this.GXG = true;
            ((PasswordFieldValidated)this.GXE).textProperty().set(var1);
         } else {
            ((PasswordFieldValidated)this.GXE).validProperty().set(var3 && var2.length() >= this.GXF);
            if (this.GXG) {
               this.GXG = false;
            } else {
               super.IFV(var1, var2);
         ...
         ((PasswordFieldValidated)this.GXE).validProperty().set(false);
      }
